...
const {findOne} = require('../../models/LiveSound');
const {defineAccess} = require('../../functional/customAccess');
...
module.exports.get_list_live_sound_ios = async function (req, res) {
  try {
    ...
    let data = await LiveSound.find(filter, null, {
      skip: page,
      limit: limitPageData,
    });
    ...
    res.status(201).json({
      ...
      page,
      count_page,
      end_page: count_page <= page + limitPageData,
      access,
    });
  } catch (e) {
    errorHandler(res, e);
...
module.exports.get_list_meditation_ios = async function (req, res) {
  try {
    ...
    let data = await Video.find(filter, null, {
      skip: page,
      limit: limitPageData,
    });
...
module.exports.get_list_live_sound = async function (req, res) {
  try {
    const page = Number(req.params.page);
    const data = await LiveSound.find({}, null, {
      skip: page,
      limit: limitPageData,
    });
    const count_page = await LiveSound.find({}).count();
...
module.exports.get_list_meditation = async function (req, res) {
  try {
    const page = Number(req.params.page);
    const data = await Video.find({category: 'meditation'}, null, {
      skip: page,
      limit: limitPageData,
    });
...
module.exports.get_list_classic = async function (req, res) {
  try {
    const page = Number(req.params.page);
    const data = await Video.find({category: 'classic'}, null, {
      skip: page,
      limit: limitPageData,
    });
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      if (!(!data[i].access || data[i].access.indexOf(access) !== -1)) {
        ...
        if (
          data[i].access.indexOf('Премиум') !== -1 &&
          data[i].access.indexOf('Без регистрации') === -1 &&
...
module.exports.complete_test = async function (req, res) {
  try {
    ...
    result.result = {
      description: resultation
        ? resultation.description
        : [
            {
              language: 'ru',
              value: 'Нет данных',
...
module.exports.get_v2_list_meditation_ios = async function (req, res) {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (check?._id) {
        let status = await LikeVideo.findOne({
          id_root: data[i]._id.toString(),
...
module.exports.get_v2_list_audio_ios = async function (req, res) {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (check._id) {
        let status = await LikeAudio.findOne({
          id_root: data[i]._id.toString(),
          id_user: check?._id.toString(),
        });
        data[i].like = status ? 1 : 0;
...
module.exports.video_like = async function (req, res) {
  try {
    ...
    const like = await LikeVideo.findOne({
...
module.exports.audio_like = async function (req, res) {
  try {
    ...
    const like = await LikeAudio.findOne({
      id_root: req.body.id.toString(),
      id_user: check?._id.toString(),
    });
...
module.exports.get_playlist = async function (req, res) {
  // В объекте каждого playlist есть settings.access (type Boolean)
  // которое определяет доступен ли плейлист для текущего пользователя
  try {
    const check = await checkUser.check(req, res);
    let data = await Playlist.find();

    data = await defineAccess(data, check, 'playlist');
...
module.exports.count_playlist = async function (req, res) {
  try {
    let data = await Playlist.findById(req.body._id);
    data.counter_start++;
    await data.save();
    res.status(201).json('OK');
  } catch (e) {
...
module.exports.get_video_ids = async function (req, res) {
  // Получить массив Video по массиву id
  try {
    const check = await checkUser.check(req, res);
    const ids = req.body.ids;
    let data = await Video.find({_id: {$in: ids}});
    if (check?._id) {
      for (let i = 0; i < data.length; i++) {
        let status = await LikeVideo.findOne({
          id_root: data[i]._id.toString(),
...
module.exports.get_audio_ids = async function (req, res) {
  // Получить массив Audio по массиву id
  try {
    const check = await checkUser.check(req, res);
    const ids = req.body.ids;
    const data = await Audio.find({_id: {$in: ids}});
    if (check?._id) {
      for (let i = 0; i < data.length; i++) {
        let status = await LikeAudio.findOne({
          id_root: data[i]._id.toString(),
