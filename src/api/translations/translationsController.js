...
module.exports.get_translations = async function (req, res) {
  try {
    ...
    await fs.readFile(
      `./${status === 'app' ? translation?.translations_app : translation?.translations_admin}`,
      'utf8',
      async function (err, data) {
        if (err) {
          console.error('errr, not File:', `./${translation?.translations_file}`);
          res.status(201).json({});
          return;
        }
...
module.exports.translation_create = async function (req, res) {
  try {
    let translations = req.body.data?._id ? await TranslationsLeng.findOne({_id: req.body.data._id}) : null;

    if (translations) {
      Object.entries(req.body.data).forEach((item) => {
        translations[item[0]] = !_.isUndefined(item[1]) ? item[1] : null;
      });
    } else {
...
module.exports.translation_list_edit = async function (req, res) {
  try {
    for (let i = 0; i < req.body.data.length; i++) {
      let translations = req.body.data[i]?._id ? await TranslationsLeng.findOne({_id: req.body.data[i]._id}) : null;
