...
const sendEmail = async function (email, message, res) {
  ...
  await emailTransport.sendMail(mailOptions, function (error, response) {
    if (error) {
      console.error('emailTransport-', error);
      res.end('error');
    }
  });
};
  ...
  if (candidate) {
    if (!candidate.isNoCheck) {
      ...
      const message_ =
        ...
        '<br><br>По всем вопросам Вы можете обращаться в службу поддержки – info@koleznev.by или по телефону/вайбер/телеграм +375(44)464-73-47.' +
        '<br>--' +
        '<br>www.koleznev.by';
