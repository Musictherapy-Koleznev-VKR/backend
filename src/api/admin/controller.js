...
const customAccess = require('../../functional/customAccess');
...
module.exports.translation = async function (req, res) {
  try {
    ...
    await fs.readFile(`./${translation[language]}`, 'utf8', async function (err, data) {
      if (err) {
        console.error('errr, not File:', `./${translation[language]}`);
        res.status(201).json({});
        return;
      }
...
module.exports.create_test = async function (req, res) {
  try {
    ...
    if (dataTable && dataTable.number) dataTable.number = dataTable.number + 1;
...
module.exports.get_list_live_sound = async function (req, res) {
  try {
    ...
    const filter = req.params.label
      ? req.params.label !== 'null'
        ? {
            label: {
              $elemMatch: {value: {$regex: req.params.label}},
            },
          }
        : {}
      : {};
...
module.exports.get_list_audio = async function (req, res) {
  try {
    ...
    if (req.params.label && req.params.label !== 'null')
      filter.label = {
        $elemMatch: {value: {$regex: req.params.label}},
      };
    ...
    for (let i = 0; i < data?.length; i++) {
      ...
      data[i].counter_start = await LogData.find({
        id_data: data[i]._id.toString(),
      }).count();
      data[i].like_tooltip = await LikeAudio.find({id_root: data[i]._id.toString()}, {date: 1, user_name: 1});
...
module.exports.get_list_video = async function (req, res) {
  try {
    ...
    for (let i = 0; i < data?.length; i++) {
      ...
      data[i].like_tooltip = await LikeVideo.find({id_root: data[i]._id.toString()}, {date: 1, user_name: 1});
      data[i].like = data[i].like_tooltip?.length;
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (data[i].info_tooltip && data[i].info_tooltip?.length && data[i].length_lessons !== 0) {
        for (let j = 0; j < data[i].info_tooltip?.length; j++) {
          data[i].info_tooltip[j].proc_lessons =
            Math.round(
              ((await UserLessonCourse.find({
                user_course_id: data[i].info_tooltip[j]._id,
                user_id: data[i].info_tooltip[j].user_id,
              }).count()) /
                data[i].length_lessons) *
                100 *
                100
            ) / 100;
        }
    ...
    for (let index = 0; index < tests.length; index++) {
      ...
      if (item_result && item_result.length) {
        ...
        for (let j = 0; j < item_result?.length; j++) {
          data_list = JSON.parse(
            JSON.stringify(
              await QuestionsTest.find({
          ...
          item_result[j].test = await Test.findOne({
            _id: item_result[j].test_id,
          });
        }
        result_data.push({
...
module.exports.users_sort = async function (req, res) {
  try {
    ...
    let filter = req.body.data.full_name
      ? req.body.data.full_name !== 'null'
        ? {fullName: {$regex: req.body.data.full_name}}
        : {}
      : {};
    ...
    let data = await User.find(filter, null, {
      skip: page,
      limit: limitPageDataVeb,
    }).sort({[req.body.sortData.value]: req.body.status ? 1 : -1});
    const count_page = Math.ceil((await User.find(filter).count()) / limitPageDataVeb) - 1;
    const count_data = await User.find().count();

    for (let i = 0; i < data.length; i++) {
      data[i].questionnaire = await Questionnaire.findOne({
        id_user: data[i]._id.toString(),
      });
      data[i].counter_video = await LogData.find({
        id_user: data[i]._id.toString(),
        type: 'video',
      }).count();
      data[i].counter_audio = await LogData.find({
        id_user: data[i]._id.toString(),
        type: 'audio',
      }).count();
    }
    ...
    res.status(201).json({
      data,
      page: Number(req.body.data.page),
      count_page,
      end_page: count_page <= page,
      count_data,
    });
...
module.exports.delete_test = async function (req, res) {
  try {
    ...
    await fs.unlink(`./${delete_data.poster}`, (err) => {
      if (err) console.error('no delete!!!!');
    });
...
module.exports.playlist_re = async function (req, res) {
  try {
    const check = await checkAdmin.check(req, res);
    if (!check.id) {
      return res.status(401).json('Unauthorized');
    ...
    if (!_.isUndefined(req.body._id)) {
      new_data = await Playlist.findOne({_id: req.body._id});
      Object.entries(req.body.data).forEach((item) => {
        new_data[item[0]] = item[1];
      });
    } else {
      new_data = new Playlist({
        ...req.body.data,
        counter_start: 0,
      });
    }
    ...
    await customAccess.customAccess(new_data, 'playlist', req.body.data.custom_access);
...
module.exports.get_playlist = async function (req, res) {
  try {
    const check = await checkAdmin.check(req, res);
    if (!check.id) {
      return res.status(401).json('Unauthorized');
    ...
    const filter = req.params.label
      ? req.params.label !== 'null'
        ...
        : {}
      : {};
    let data = await Playlist.find(filter);
    for (let i = 0; i < data.length; i++) {
      data[i].custom_access = await customAccess.customAccessGet(data[i], 'playlist');
      data[i].audio_l = data[i].ids_audio ? data[i].ids_audio.length : 0;
      data[i].video_l = data[i].ids_video ? data[i].ids_video.length : 0;
    }
    const count_page = Math.ceil((await Playlist.find(filter).count()) / limitPageDataVeb) - 1;
...
module.exports.get_audio_to_playlist = async function (req, res) {
  try {
    const data = await Audio.find(null, 'label');
    res.status(201).json(data);
  } catch (e) {
    errorHandler(res, e);
  }
...
module.exports.get_video_to_playlist = async function (req, res) {
  try {
    const data = await Video.find(null, 'label_');
    res.status(201).json(data);
  } catch (e) {
    errorHandler(res, e);
  }
...
module.exports.delete_playlist = async function (req, res) {
  try {
    await Playlist.deleteOne({_id: req.body._id});
    await customAccess.deleyteAccesses(req.body._id, 'playlist');
    res.status(201).json('OK');
  } catch (e) {
    errorHandler(res, e);
  }
...
module.exports.get_users_to_access = async function (req, res) {
  try {
    let list_mus = [];
    let musictherapys = await User.find(null, 'name fullName');
    for (let i = 0; i < musictherapys.length; i++) {
      list_mus.push({
        id: musictherapys[i]._id,
        name: musictherapys[i].name + ' ' + musictherapys[i].fullName,
...
module.exports.get_users_name_to_id = async function (req, res) {
  try {
    const data = await User.findOne({_id: req.body.id.toString()});
    const user = {
...
module.exports.get_audio_label_to_id = async function (req, res) {
  try {
    const data = await Audio.findOne({_id: req.body.id}, 'label');
    res.status(201).json(data);
  } catch (e) {
    errorHandler(res, e);
  }
...
module.exports.get_video_label_to_id = async function (req, res) {
  try {
    const data = await Video.findOne({_id: req.body.id}, 'label_');
    res.status(201).json(data);
  } catch (e) {
    errorHandler(res, e);
  }
